#ifndef CaPTkROIConstructionHelper_h
#define CaPTkROIConstructionHelper_h
...
public:
    ...
    ~ROIConstructionHelper() {  }

    void Update(
        float radius,
        float step) override
    {
        m_Indeces = captk::ROIConstructionCreateLatticePoints<TPixel, VImageDimension>(
            m_Mask, 
            step,
            (m_Mode == ROI_BASED) ? true : false
        );

        m_CurrentIndex = 0;

        SpacingType spacing = m_Mask->GetSpacing();
        ...
        m_Iter  = itk::ImageRegionIteratorWithIndex<ImageType>(
            m_Mask, m_Mask->GetRequestedRegion()
        );
    }
...
#endif // ! CaPTkROIConstructionHelper_h